...
if (mysqli_num_rows($result) > 0) {
    ...
    if ($row["email"] == $registerEmail) {
        //overuje zda je to stejny jestli ano tak to posle ze uz je registered
        header("Location:../sites/errorsite/userExist.php");
    }
} else {
    $stmt = $conn->prepare(
        "INSERT INTO `users` (`id`, `username`, `email`, `password`) VALUES (NULL, '$registerUsername', '$registerEmail', '$registerPassword');",
    );
    $stmt->execute();
